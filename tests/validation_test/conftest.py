...
def test_database_url():
    ...
    if not url.database or not url.database.endswith("_test"):
        raise RuntimeError("Validation tests require a PostgreSQL database ending in '_test'.")
    if not re.fullmatch(r"[A-Za-z0-9_]+", url.database):
        raise RuntimeError("The validation test database name contains unsupported characters.")
    return url
...
@pytest.fixture()
def db_session(validation_engine):
    Base.metadata.drop_all(bind=validation_engine)
    Base.metadata.create_all(bind=validation_engine)
    session = sessionmaker(bind=validation_engine, autoflush=False, expire_on_commit=False)()
    try:
        yield session
    finally:
        session.close()
...
def provider_payload():
    def build(role, suffix):
        payload = {
            ...
            "name": f"Validation Provider {suffix}",
            "phone": f"0788111{suffix:03d}",
            "district": "Kamonyi",
            "sector": "Runda",
            "cell": "Gacurabwenge",
            "village": "Kigusa",
        ...
        if role == "truck_provider":
            payload.update({"plate_number": f"RAB {suffix:03d}B", "capacity_kg": 1000})
        else:
            payload.update({"total_capacity_kg": 2000})
        return payload
